...
sunDetector::sunDetector()
    : sunCenter(-1, -1), detectedArea(0.0), sunFound(false) {}

void sunDetector::startVideo(const cv::Mat &frame) {
  currentFrame = frame.clone();
}

void sunDetector::endVideo() { currentFrame.release(); }
...
void sunDetector::findSun(const cv::Mat &frame) {
  ...
  for (const auto &contour : contours) {
    ...
    if (area > maxArea) {
      ...
      if (m.m00 != 0) {
        maxCenter = cv::Point(static_cast<int>(m.m10 / m.m00),
...
std::pair<int, int> sunDetector::getSunCoordinates() const {
  return sunFound ? std::make_pair(sunCenter.x, sunCenter.y)
                  : std::make_pair(-1, -1);
}

double sunDetector::getDetectedArea() const { return detectedArea; }

void sunDetector::drawSun(cv::Mat &frame) const {
  if (sunFound) {
    int radius =
        std::clamp(static_cast<int>(sqrt(detectedArea / CV_PI)), 10, 100);
