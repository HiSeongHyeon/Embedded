...
// 차량 내 카메라 위치 (미터 단위): (x, y, z) - 업데이트된 값 반영
const cv::Point3d DEFAULT_CAMERA_POS(0.5, 1.0, 0.5); // y와 z가 이전과 다름

// 운전자 눈 위치 (미터 단위): (x, y, z) - 업데이트된 값 반영
const cv::Point3d DEFAULT_DRIVER_POS(0.25, 1.2, 0.0); // y와 z가 이전과 다름

// 운전자 좌표계에서 앞유리(윈드실드) 평면의 z 좌표 (미터 단위) - 업데이트됨
...
// 윈드실드(유리창) 좌상단 물리 좌표 (미터 단위): (x_left, y_top) - 변수명 및
// 주석 업데이트
